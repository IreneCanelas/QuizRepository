...
session_start(); 
    //Incluindo a conexão com a base de dados   
	include "connection.php";
	include "registos.php";    
    //O campo usuário e senha preenchido entra no if para validar
    if((isset($_POST['email'])) && (isset($_POST['psw']))){
        $email = mysqli_real_escape_string($conn, $_POST['email']); 
        $psw = mysqli_real_escape_string($conn, $_POST['psw']);
            
        //Buscar na tabela registos o username que corresponde com os dados digitado no formulário
        $result_registos = "SELECT * FROM registos WHERE email = '$email' && psw = '$psw';
